...
4. Select **Deploy → Manage deployments**, edit the existing Web app, select **New version**, and deploy it. Set **Execute as** to yourself and **Who has access** to Anyone.
5. The current Web app URL is already set in `coming-soon.js`; do not change it unless Google gives you a new `/exec` URL.

Each valid submission appends one simple export-ready row. If the Apps Script deployment is updated later, redeploy it and replace the URL only if Google provides a new one.
